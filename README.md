...
- 支付：模拟网关（测试用）、易支付协议、支付宝（当面付扫码 / 电脑网站支付 / 手机网站支付）、微信支付（Native 扫码 / H5），后台可配置
...
登录管理后台 `/admin` → 支付网关，新增“支付宝”实例，填写应用 ID、应用私钥和支付宝公钥。应用公钥需先上传到支付宝开放平台；支付宝公钥用于服务器验证异步回调，两者不是同一把公钥。在线支付手续费率可在每个网关实例中单独设置（0 到 100%，按分四舍五入）；支付页会按所选网关的费率向用户加收，余额支付不收手续费。在线充值的手续费由用户承担，但充值到账仍为充值本金。

每个实例的“支付模式”决定付款形态：
- **扫码模式**（支付宝默认）：电脑端显示本站二维码结算页（支付宝走当面付预下单）。手机端会自动改走「手机网站支付」跳转，避免用户在自己手机上看到无法扫描的二维码；未签约「手机网站支付」时可开启「手机端也扫码」回退为二维码。易支付该模式调用上游 API 取二维码。
- **跳转模式**（易支付默认）：直接打开上游收银台。支付宝按浏览器 UA 选择电脑网站支付 / 手机网站支付；易支付提交到上游托管收银台。

支付宝的跳转模式与手机端跳转都需在开放平台签约对应产品（电脑网站支付 / 手机网站支付），否则收银台会报 ISV 权限不足。

异步回调地址为 `{base_url}/pay/notify?code=网关编码`，请确保支付宝平台能访问。支付宝应用需配置 RSA2，并将回调地址配置为该地址。

### 微信支付

新增“微信支付”实例，需填写 7 项凭据：微信 AppID、商户号、商户 API 私钥、APIv3 密钥（32 位）、商户证书序列号、微信支付公钥及其公钥 ID。微信自 2024-10 起不再为新商户提供平台证书下载，因此本项目只支持**公钥模式**验签。

微信支付没有“电脑端网页跳转收银台”形态，也**没有支付模式开关**，按设备自动选择：电脑端走 Native 扫码（本站二维码结算页），手机端走 H5 支付跳转。微信内置浏览器两者都不可用（需 JSAPI + 公众号授权，尚未实现），此时会提示用户改用浏览器打开。

回调地址为 `{base_url}/pay/网关编码/notify`——微信明确禁止 `notify_url` 携带查询参数，故改用路径段承载网关编码。该地址必须为公网可访问的 **HTTPS**。应答体为 `{"code":"SUCCESS","message":"成功"}`，否则微信会持续重试。
