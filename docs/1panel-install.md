...
需添加一个环境变量，否则安装向导无法从公网访问：

```text
INSTALL_LISTEN=:8080
```

原因：安装阶段（未生成 `config.yaml` 时）程序默认只监听本机回环地址 `127.0.0.1:8080`，
容器端口映射无法转发到它，`http://服务器IP:8080/install` 会打不开。设置该变量后安装
向导监听所有网卡；安装完成重启后正式程序读取 `config.yaml` 中的 `listen: ":8080"`，
此变量不再需要（保留也无副作用）。若你通过 1Panel 终端配合 SSH 端口转发访问安装页，
则不设置此变量也能完成安装。
...
创建并启动应用后，打开（**前提**：已按上节设置环境变量 `INSTALL_LISTEN=:8080`，否则公网打不开，见第 7 节排查第 10 条）：
