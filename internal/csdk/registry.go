...
// Factory 适配器工厂：由宿主（设置读取 + HTTP 客户端）构造 Provider。
...
// Register 注册验证码供应商（描述符 + 工厂单入口，适配器包 init 调用）。
...
func Register(d Descriptor, f Factory) {
	...
	if key == "" {
		panic("csdk: 供应商缺少 Key")
	}
	registryMu.Lock()
	defer registryMu.Unlock()
	if _, dup := registry[key]; dup {
		panic(fmt.Sprintf("csdk: 供应商重复注册: %s", key))
	}
...
// Registry 返回全部已注册供应商描述符（key → 描述符，拷贝）。
...
// New 按 key 创建适配器实例；found=false 表示未注册（无错误）。
