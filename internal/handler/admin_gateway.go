...
type adminGatewayRow struct {
	ID                                                                                                int64
	Code, Driver, Name, APIURL, PID, Channel, PaymentMode, MobileQRCode, AppID, PrivateKey, PublicKey string
...
func (g *AdminGateway) form(w http.ResponseWriter, r *http.Request) {
	...
	for _, v := range list {
		...
		rows = append(rows, adminGatewayRow{ID: v.ID, Code: v.Code, Driver: v.Driver, Name: v.Name,
			APIURL: v.Config["api_url"], PID: v.Config["pid"], Channel: v.Config["channel"], PaymentMode: v.Config["payment_mode"], MobileQRCode: v.Config["mobile_qrcode"], AppID: v.Config["app_id"], PrivateKey: v.Config["private_key"], PublicKey: v.Config["public_key"], Key: v.Config["key"], FeePercent: feePercent, Enabled: v.Enabled, Sort: v.Sort})
	}
	...
	for _, v := range rows {
		out = append(out, map[string]any{
			...
			"api_url": v.APIURL, "pid": v.PID, "channel": v.Channel, "payment_mode": v.PaymentMode,
			"mobile_qrcode": v.MobileQRCode, "app_id": v.AppID,
...
func (g *AdminGateway) save(w http.ResponseWriter, r *http.Request) {
	...
	cfg := map[string]string{
		...
		"mobile_qrcode": strings.TrimSpace(fv("mobile_qrcode")),
