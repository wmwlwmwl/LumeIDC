...
func FulfillmentLockKey(serviceID int64) string {
	return fmt.Sprintf("fulfillment-service:%d", serviceID)
}
...
func (r *FulfillmentJobs) TryExecutionLock(ctx context.Context, serviceID int64) (*sql.Conn, func(), error) {
	...
	if err = conn.QueryRowContext(ctx, `SELECT pg_try_advisory_lock(hashtextextended($1,0))`, key).Scan(&locked); err != nil || !locked {
		...
		return nil, nil, ErrFulfillmentBusy
	}
	return conn, func() {
...
func (r *FulfillmentJobs) ValidateClaim(ctx context.Context, conn *sql.Conn, job *FulfillmentJob) error {
	var valid bool
	err := conn.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM fulfillment_jobs WHERE id=$1 AND service_id=$2
