...
func NewSMSProvider(key string, settings map[string]string, client *http.Client) (SMSProvider, error) {
	...
	if provider, found, err := smsProviderFromRegistry(key, settings, client); found {
		return provider, err
	}
	// 旧实现回退（逐步迁移到新适配器后删除）。
...
func smsDoRequest(ctx context.Context, client *http.Client, method, endpoint string, body []byte, headers map[string]string) ([]byte, error) {
	// 包装 client，强制禁用重定向跟随（与旧 ConfiguredSMSProvider.client() 行为一致）。
	safeClient := *client
...
func (p *configuredSMSProvider) request(ctx context.Context, method, endpoint string, body []byte, headers map[string]string) ([]byte, error) {
	return smsDoRequest(ctx, p.client, method, endpoint, body, headers)
}
