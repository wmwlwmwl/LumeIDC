...
<script setup lang="ts">
  ...
  // 这些类型的 options 只用于下方插槽渲染 ElOption/ElCheckbox/ElRadio，
  // 不能再透传给组件（否则会在 DOM 上留下 options="[object Object]" 这种怪属性）。
  const optionLoopTypes = ['select', 'checkboxgroup', 'radiogroup']
  const getBindProps = (item: SearchFormItem) => {
    const itemProps = getProps(item)
    if (item.type && optionLoopTypes.includes(item.type) && itemProps && 'options' in itemProps) {
      const { options: _options, ...rest } = itemProps as Record<string, any>
      return rest
    }
    return itemProps
  }
