...
const appConfig: SystemConfig = {
  // 系统信息（默认值，启动后由 /session 的站点信息覆盖，见 config/site.ts）
  systemInfo: {
    name: 'LumeIDC'
  },
  // 系统主题
 ...
 * 系统全局配置（响应式）。
 *
 * 站点名称等需要在运行期被 /session 覆盖，因此这里保持 reactive 而不是冻结，
 * 模板中直接读取 `AppConfig.systemInfo.name` 即可获得响应式更新。
 */
const AppConfig = reactive(appConfig)

/**
 * 用 LumeIDC /session 下发的站点信息覆盖默认系统信息。
 */
export function applySiteInfo(site?: { name?: string }): void {
  const name = (site?.name || '').trim()
  if (name) {
    AppConfig.systemInfo.name = name
  }
}
